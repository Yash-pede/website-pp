...
const page = async ({ params }: { params: Promise<{ slug: string }> }) => {
  ...
  const formattedName =
    slug === undefined
      ? ""
      : slug.includes("%2B") // for handling
      ? slug.replace("%2B", "+") // '+' in the URL
      : slug;
  ...
  return products.map((product, index) => {
    if (
      ...
      return (
        <main className="flex min-h-screen flex-col items-center justify-between" key={index}>
          <section className="mt-[135px] w-full max-sm:mt-[80px]" >
            <GreenOverlay />
                ...
                <div>
                  ...
                  <ul className="flex flex-col gap-2">
                    {product.uses.map((point, index) => (
                      <li
                        className="roboto text-[16px] text-[#555] "
                        key={index}
                      >
                        {point}
                      </li>
